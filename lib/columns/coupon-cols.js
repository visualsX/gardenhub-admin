...
export function CouponCols() {
  return [
    ...
    },
    {
      ...
      key: 'actions',
      width: 100,
      render: (_, record) => <CouponActions record={record} />,
    },
  ];
}

// Inline actions component for coupons
function CouponActions({ record }) {
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} className="flex items-center gap-3">
      ...
      <button
        onClick={() => router.push(`/coupons/edit?id=${record.id}`)}
