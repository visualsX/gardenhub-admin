...
import {
  ...
  useUpdateFeaturedCategoryImage,
      ...
      <Form form={form} layout="vertical" onFinish={handleOk} className="py-2" requiredMark={false}>
        ...
        <div className="mb-6">
          <h2 className="text-xl font-semibold">
            {isEdit ? 'Edit Featured Category' : 'Add Featured Category'}
          </h2>
          <p className="mt-1 text-sm text-gray-500">
            {isEdit
              ? 'Update existing featured category details'
              : 'Configure a new featured category for the landing page'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-x-4 md:grid-cols-2">
          ...
          />
          <FormInputNumber name="displayOrder" label="Display Order" min={0} className="w-full" />
        </div>
        ...
        <div className="mt-8 flex justify-end gap-3">
          ...
          <Button
            ...
            loading={isLoading}
            disabled={isLoading}
            className="h-[45px] rounded-lg bg-green-600 px-8 hover:bg-green-700"
          >
