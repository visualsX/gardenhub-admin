...
import AttributeModal from '@/components/pages/configurations/attributes/modals/AttributeModal';
import OptionModal from '@/components/pages/configurations/attributes/modals/OptionModal';
...
export default function AttributesManagement() {
  ...
  const { openDeleteModal, closeDeleteModal, openModal, isModalOpen, isDeleteModalOpen } =
    useUiStates();
  ...
  if (isLoading) {
    return (
      <Box
        ...
        extra={
          <Button type="primary" icon={<PlusWhite />} onClick={handleAddAttribute}>
            Add Attribute
          </Button>
        }
      >
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        {attributes?.map((attr) => (
          <div key={attr.id} className="overflow-hidden rounded-lg border border-gray-200 bg-white">
            {/* Attribute Row */}
            <div
            ...
            >
              ...
              <div className="flex items-center gap-8">
                <span className="text-sm text-gray-600">{attr.options?.length || 0} options</span>
                <Button
                  size="small"
                  icon={<PlusGreen />}
                ...
                <button className="flex h-8 w-8 items-center justify-center text-gray-600 hover:text-gray-900">
                  <Edit className="h-5 w-5" onClick={(e) => handleEditAttribute(e, attr)} />
                </button>
                <button className="flex h-8 w-8 items-center justify-center text-gray-600 hover:text-gray-900">
                  <Trash className="h-5 w-5" onClick={(e) => handleDeleteAttribute(e, attr)} />
                </button>
              </div>
            </div>
                  ...
                  attr.options.map((option) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-8">
                        ...
                        <button className="flex h-8 w-8 items-center justify-center text-gray-600 hover:text-gray-900">
                          <Trash className="h-5 w-5" onClick={() => handleDeleteOption(option)} />
                        </button>
                      </div>
                    </div>
                  ))
