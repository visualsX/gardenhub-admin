...
import VariantEditModal from '@/components/pages/products/edit/VariantEditModal';
    ...
    <div className="space-y-6">
      <Box header title={'All Variants'} description={'Basic details about the product'}>
        <div className="mb-6 flex gap-3">
          {editPage ? (
            <>
              <Button type="primary" className="bg-[#1B5E20]" onClick={() => setIsEditModalVisible(true)}>
                Manage Variants
              </Button>
              <VariantEditModal
                ...
                productId={productId}
                initialVariants={form.getFieldValue('Variants')}
                options={form.getFieldValue('Options')}
                mainSku={form.getFieldValue('Sku')}
              />
            </>
          ) : (
            <>
...

        {
          editPage ? (
            <div className="space-y-4">
              {form.getFieldValue('Variants')?.map((variant, index) => (
                <div key={index} className="rounded-lg border border-gray-200 bg-white p-4">
                  <div className="flex items-center justify-between">
                    <span className="font-medium">{variant.name}</span>
                    <div className="text-gray-500">
                      <span className="mr-4">SKU: {variant.sku}</span>
                      <span>Price: {variant.price}</span>
                    </div>
                  </div>
                </div>
              ))}
              {(!form.getFieldValue('Variants') || form.getFieldValue('Variants').length === 0) && (
                <div className="text-gray-500 italic">No variants available. Click "Manage Variants" to add.</div>
              )}
            </div>
          ) : (
