...
const ShippingTab = ({ order, isLoading }) => {
  const timelineItems = order?.trackingHistory?.map((history) => ({
    color: 'green',
    children: (
      <div>
        ...
        >
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <LabelAndValue label="Tracking Number" value={order?.trackingNumber || 'Not available'} />
              <LabelAndValue 
                label="Expected Delivery" 
                value={order?.expectedDeliveryDate ? new Date(order.expectedDeliveryDate).toLocaleDateString() : '-'} 
              />
            </div>
