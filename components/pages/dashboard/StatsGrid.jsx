...
const StatsGrid = ({ stats, loading }) => {
  ...
  const statsItems = [
    {
      label: 'Total Sales',
      value: `AED ${stats?.totalSales?.toLocaleString() || '0'}`,
    ...
    <div className="mb-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
      {statsItems.map((stat, i) => (
        <div key={i} className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          <div
            className={`mt-2 text-2xl font-bold ${stat.isAlert && stat.value > 0 ? 'text-red-600' : stat.isWarning && stat.value > 0 ? 'text-amber-600' : 'text-gray-900'}`}
          >
            {stat.value}
          </div>
          {stat.change !== undefined && (
            <div
              className={`mt-2 flex items-center text-xs font-medium ${stat.change >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
              {stat.change >= 0 ? (
