...
export const useBanners = (filters = {}) => {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await graphqlClient.request(LANDING_PAGE_QUERIES.GET_BANNERS, variables);
  ...
  return useMutation({
    mutationFn: async (data) => {
      const response = await apiClient.put(
        API_ENDPOINTS.LANDING_PAGE.BANNERS.UPDATE(data?.id),
        data
      );
      return response;
    },
  ...
  return useMutation({
    mutationFn: async ({ id, formData }) => {
      const response = await apiClient.post(
        API_ENDPOINTS.LANDING_PAGE.BANNERS.UPDATE_IMAGES(id),
        formData,
        {
          headers: {
...
export const useFeaturedCategories = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await graphqlClient.request(LANDING_PAGE_QUERIES.GET_FEATURED_CATEGORIES);
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await graphqlClient.request(
        LANDING_PAGE_QUERIES.GET_FEATURED_CATEGORY_BY_ID,
        {
          id: +id,
        }
      );
...
export const useCreateFeaturedCategory = () => {
  ...
  return useMutation({
    mutationFn: async (data) => {
      const response = await apiClient.post(
        API_ENDPOINTS.LANDING_PAGE.FEATURED_CATEGORIES.CREATE,
        data
      );
      return response;
    },
...
export const useUpdateFeaturedCategory = () => {
  ...
  return useMutation({
    mutationFn: async (data) => {
      const response = await apiClient.put(
        API_ENDPOINTS.LANDING_PAGE.FEATURED_CATEGORIES.UPDATE(data?.id),
        data
      );
      return response;
    },
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: landingPageKeys.featuredCategoryDetail(variables.id),
      });
  ...
  return useMutation({
    mutationFn: async (id) => {
      const response = await apiClient.delete(
        API_ENDPOINTS.LANDING_PAGE.FEATURED_CATEGORIES.DELETE(id)
      );
      return response;
    },
    onSuccess: () => {
...
export const useUpdateFeaturedCategoryImage = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, formData }) => {
      const response = await apiClient.post(
        API_ENDPOINTS.LANDING_PAGE.FEATURED_CATEGORIES.UPDATE_IMAGE(id),
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
