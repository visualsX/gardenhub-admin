...
export const useShippingZones = (filters = {}) => {
    ...
    const query = useQuery({
        ...
        queryFn: async () => {
            ...
            const response = await graphqlClient.request(SHIPPING_ZONE_QUERIES.GET_SHIPPING_ZONES, variables);
...
export const useShippingRate = (id) => {
    return useQuery({
        ...
        queryFn: async () => {
            if (!id) return null;
            const response = await graphqlClient.request(SHIPPING_ZONE_QUERIES.SHIPPING_RATE_BY_ID, { id });
    ...
    return useMutation({
        ...
        onSuccess: (_, variables) => {
            ...
            if (variables.shippingZoneId) {
                queryClient.invalidateQueries({ queryKey: shippingKeys.zoneDetail(variables.shippingZoneId) });
            }
    ...
    return useMutation({
        ...
        onSuccess: (_, variables) => {
            if (variables.data.shippingZoneId || variables.shippingZoneId) {
                queryClient.invalidateQueries({ queryKey: shippingKeys.zoneDetail(variables.data.shippingZoneId || variables.shippingZoneId) });
            }
