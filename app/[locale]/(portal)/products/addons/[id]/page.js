  ...
  return (
    <div className="space-y-6">
      ...
      <GoBack
        ...
        title={'Addon Details'}
        desc={'View addon details and manage option images'}
      />
      {/* Addon Information */}
      ...
      <Box header title="All Addon Options" description="Manage images for each option">
        {addon.options && addon.options.length > 0 ? (
              ...
              <Box header title={'Option ' + (index + 1)} key={option.id || index}>
                <section className="grid grid-cols-[150px_1fr] gap-4">
                  <Box classRest="col-span-1">
                    <label className="mb-2 block text-sm font-semibold">Option Image</label>
                    {option.id ? (
                      <AddonOptionImageUploader optionId={option.id} imageUrl={option.imageUrl} />
                    ) : (
                      <div className="inline-block rounded border bg-white px-3 py-2 text-sm text-gray-400">
                  ...
                  <Box classRest="grid grid-cols-2 2xl:grid-cols-3 gap-4">
                    ...
                    <div>
                      <label className="mb-1 block text-sm font-semibold">Default Option</label>
          ...
          <div className="py-8 text-center text-gray-500">No options available for this addon</div>
        )}
      </Box>
    </div>
  );
