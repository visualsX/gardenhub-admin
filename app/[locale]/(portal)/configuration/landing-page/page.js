...
export default function LandingPageConfigPage() {
  ...
  const { isDeleteModalOpen, openDeleteModal, openModal, openDetailModal } = useUiStates();
  ...
  const {
    ...
    pageState: bannerPageState,
  } = useBanners({
    paginationKey: PAGINATION_KEYS.LANDING_PAGE,
    pageSize: DEFAULT_CURSOR_PAGE_SIZE,
    where: searchTerm
      ? {
          or: [{ heading: { contains: searchTerm } }, { subheading: { contains: searchTerm } }],
        }
      : null,
  });
  ...
  const items = [
    {
      ...
      children: (
        <div className="space-y-6">
          <LandingPageFilters searchTerm={searchTerm} onSearchChange={setSearchTerm} />
          <DataTable
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              Manage banners and featured category sections
            </p>
          </div>
          ...
          {activeTab === 'banners' ? (
            ...
            <Button
              ...
              className="bg-primary hover:bg-primary/80 flex h-[42px] items-center gap-2 rounded-lg px-6"
            >
      ...
      <DeleteModal
        loading={
          activeTab === 'banners' ? deleteBanner?.isPending : deleteFeaturedCategory?.isPending
        }
