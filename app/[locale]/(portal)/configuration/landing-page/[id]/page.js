...
export default function BannerDetailPage() {
    ...
    const handleImageUpdate = (type) => {
        ...
        if (fileList && fileList[0]?.originFileObj) {
            ...
            updateImages.mutate({ id, formData }, {
                onSuccess: () => {
                ...
                <div className="lg:col-span-2 space-y-6">
                    <Box title="Banner Content" header>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <LabelAndValue label="Video URL" value={banner.videoUrl || 'N/A'} isLink={!!banner.videoUrl} />
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Box title="Primary Button" header>
                            <div className="space-y-4">
                                <LabelAndValue label="Text" value={banner.primaryButtonText || banner.primaryButton?.text || 'N/A'} />
                                <LabelAndValue label="Link" value={banner.primaryButtonLink || banner.primaryButton?.link || 'N/A'} />
                                <LabelAndValue label="Style" value={banner.primaryButtonStyle || banner.primaryButton?.style || 'N/A'} />
                            </div>
                        </Box>

                        <Box title="Secondary Button" header>
                            <div className="space-y-4">
                                <LabelAndValue label="Text" value={banner.secondaryButtonText || banner.secondaryButton?.text || 'N/A'} />
                                <LabelAndValue label="Link" value={banner.secondaryButtonLink || banner.secondaryButton?.link || 'N/A'} />
                                <LabelAndValue label="Style" value={banner.secondaryButtonStyle || banner.secondaryButton?.style || 'N/A'} />
                            </div>
                        </Box>
                    </div>
                    ...
                    <Box title="Design & Scheduling" header>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <LabelAndValue label="Active Period" value={
                    ...
                    <Form form={form} layout="vertical">
                        <Box title="Desktop Image" header description="Upload or update desktop banner image">
                            <SingleImageUploader
                                ...
                                label=""
                                editPage={true}
                                existingImage={banner.imageUrl ? { imageUrl: `${banner.imageUrl}?v=${refreshCounter}` } : null}
                            />
                            <Button
                                type="primary"
                                block
                        ...
                        <Box title="Mobile Image" header className="mt-6" description="Upload or update mobile banner image">
                            <SingleImageUploader
                                name="mobileImage"
                                label=""
                                editPage={true}
                                existingImage={banner.mobileImageUrl ? { imageUrl: `${banner.mobileImageUrl}?v=${refreshCounter}` } : null}
                            />
                            <Button
                                type="primary"
                                block
