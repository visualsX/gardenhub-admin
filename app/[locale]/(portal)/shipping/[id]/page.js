...
export default function ShippingZoneDetailPage() {
    ...
    if (isLoading) return <div className="p-12 text-center text-gray-500">Loading zone details...</div>;
    if (!zone) return <div className="p-12 text-center text-red-500">Zone not found</div>;

    const countryList = typeof zone.countryCodes === 'string'
    ...
    return (
        <div className="min-h-screen pb-20">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <Box title="Zone Information" header>
                        <div className="space-y-4">
                            ...
                            <div>
                                <p className="text-sm text-gray-500 mb-2">Emirates / Regions</p>
                ...
                <div className="lg:col-span-2">
                    <Box
                        ...
                        extra={<Button type="primary" onClick={handleAddRate}>Add Rate</Button>}
                    >
                        <DataTable
                            data={zone.rates || []}
                            rowKey="id"
                            pagination={false}
                            columns={[
                                { title: 'Rate Name', dataIndex: 'rateName', key: 'rateName', render: (t) => <span className="font-medium">{t}</span> },
                                { title: 'Base Cost', dataIndex: 'baseCost', key: 'baseCost', render: (v) => `$${v?.toFixed(2)}` },
                                {
                                    title: 'Free Shipping Threshold',
                                    dataIndex: 'freeShippingThreshold',
                                    key: 'freeShippingThreshold',
                                    render: (v) => v ? <span className="text-green-600">Over ${v.toFixed(2)}</span> : <span className="text-gray-400">-</span>
                                },
                                {
                                    title: 'Actions',
                                    key: 'actions',
                                    ...
                                    render: (_, record) => (
                                        <Space>
                                            <Button type="text" icon={<Edit className="h-4 w-4" />} onClick={() => handleRateEdit(record)} />
                                            <Button type="text" danger icon={<Trash2 className="h-4 w-4" />} onClick={() => setRateToDelete(record)} />
                                        </Space>
                                    )
                                }
                            ]}
            ...
            >
                <p>Are you sure you want to delete the rate <b>{rateToDelete?.rateName}</b>?</p>
            </Modal>
        </div>
    );
}
